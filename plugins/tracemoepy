...
@Client.on_message(filters.command(["rev", f"rev{BOT_NAME}"], prefixes=trg))
async def trace_bek(client: Client, message: Message):
    ...
    if dls_loc:
        ...
        button.append([InlineKeyboardButton("➡️ Next", callback_data=f"tracech_1_{dls_loc}_{message.from_user.id}")])
...
async def tracemoe_btn(client: Client, cq: CallbackQuery):
    ...
    if await check_if_adult(int(result['anilist']['id']))=="True" and await (SFW_GRPS.find_one({"id": cq.message.chat.id})):
        ...
        button.append([InlineKeyboardButton("🆕 More Info", url=f"https://anilist.co/anime/{result['anilist']['id']}")])
    if int(page)==0:
        button.append([InlineKeyboardButton("➡️ Next", callback_data=f"tracech_{int(page)+1}_{dls_loc}_{user}")])
        ...
        button.append([
            ...
            InlineKeyboardButton("➡️ Next", callback_data=f"tracech_{int(page)+1}_{dls_loc}_{user}")
        ])
    await cq.edit_message_media(msg, reply_markup=InlineKeyboardMarkup(button))


HELP_DICT["reverse"] = """Use `/rev` cmd to get reverse search via tracemoepy API
