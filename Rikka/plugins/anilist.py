...
@Client.on_message(filters.command(["favs", f"favs{BOT_NAME}"], prefixes=trg))
...
HELP_DICT["anime"] = """Use /ani cmd to get info on specific anime using
...
**Usage:**
        `/ani Fumetsu No Anata E`
        `!ani Higehiro`
        `/ani 98385`"""
HELP_DICT["anilist"] = """Use `/anilist` cmd to get info on all animes related to search query
(Don't confuse with `/ani`)

This cmd helps you choose between animes with similar names,
while `/ani` gives info on single anime

**Usage:**
        `/anilist Darling in the franxx`
        `!anilist 3D Kanojo`"""
HELP_DICT["character"] = """Use `/charac` cmd to get info on characters

**Usage:**
        `/charac Saber`
        `!charac Mai Sakurajima`"""
HELP_DICT["manga"] = """Use `/manga` cmd to get info on mangas

**Usage:**
        `/manga Karakai Jouzu no Takagi San`
        `!manga Ao Haru Ride`"""
HELP_DICT["airing"] = """Use `/air` cmd to get info on airing status of anime
