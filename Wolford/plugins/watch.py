...
async def watch_(client, cq: CallbackQuery):
    ...
    if total>50:
        if int(req)==0:
            button.append([InlineKeyboardButton(text="Next ➡️", callback_data=f"{kek}_{id_}_{qry}_{int(req)+1}_{user}")])
        elif int(req)==totalpg:
            button.append([InlineKeyboardButton(text="⬅️ Prev", callback_data=f"{kek}_{id_}_{qry}_{int(req)-1}_{user}")])
        else:
            button.append(
                [
                    InlineKeyboardButton(text="⬅️ Prev", callback_data=f"{kek}_{id_}_{qry}_{int(req)-1}_{user}"),
                    InlineKeyboardButton(text="Next ➡️", callback_data=f"{kek}_{id_}_{qry}_{int(req)+1}_{user}")
                ]
            )
